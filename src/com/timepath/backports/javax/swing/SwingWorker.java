 ...
 */
package com.timepath.backports.javax.swing;
 ...
 * Thread</i>. Otherwise the application becomes unresponsive.
 * </li>
 ...
 *
 * <p> {@code SwingWorker} is designed for situations where you need to have a
 * long running task run in a background thread and provide updates to the UI
 * either when done, or while processing. Subclasses of {@code SwingWorker} must
 ...
 * It schedules {@code SwingWorker} for the execution on a
 * <i>worker</i>
 ...
 *
 * <p> {@code SwingWorker} is only designed to be executed once. Executing a
 ...
 *       {@code @Override} public String doInBackground() { return
 * findTheMeaningOfLife(); }
 *
 * {@code @Override} protected void done() { try { label.setText(get()); } catch
 * (Exception ignore) { } } }
 *
 ...
 *     }
 *
 *     {@code @Override} public List&lt;Integer&gt; doInBackground() { while (!
 * enough &amp;&amp; ! isCancelled()) { number = nextPrimeNumber();
 * publish(number); setProgress(100 * numbers.size() / numbersToFind); } }
 * return numbers; }
 *
 * {@code @Override} protected void process(List&lt;Integer&gt; chunks) { for
 * (int number : chunks) { textArea.append(number + &quot;\n&quot;); } } }
 *
...
public abstract class SwingWorker<T, V> implements RunnableFuture<T> {
     ...
     *     }
     *
     *     {@code @Override} protected DefaultTableModel doInBackground() throws
     * Exception { for (Object[] row = loadData(); ! isCancelled() &amp;&amp;
     * row != null; row = loadData()) { publish((Object[]) row); } return
     * tableModel; }
     *
